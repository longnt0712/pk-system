...
public class QuestionDto implements Serializable  {
	...
	public void setFindExactWord(boolean findExactWord) {
		this.findExactWord = findExactWord;
	}
	...
	public QuestionDto(Question domain) {
...
//		}
	...
	public class sortByOrdinalNumber implements Comparator<QuestionDto> {
	...
	public ArrayList<QuestionDto> setListSubQuestions(Question domain) {
		if(domain.getSubQuestions()!=null && domain.getSubQuestions().size()>0) {
			ArrayList<QuestionDto> subQuestions =new ArrayList<QuestionDto>();
			for(Question q: domain.getSubQuestions()) {
				QuestionDto sDto = new QuestionDto();
				...
				if(q.getQuestionTopics() != null && q.getQuestionTopics().size() > 0) {
					sDto.questionTopics = new ArrayList<QuestionTopicDto>();
					List<QuestionTopicDto> ret = new ArrayList<QuestionTopicDto>();
					for (QuestionTopic item : q.getQuestionTopics()) {
						ret.add(new QuestionTopicDto(item));
					}
					sDto.questionTopics.addAll(ret);
				}
