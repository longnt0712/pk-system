...
(function () {
    ...
    angular.module('Hrm.EducationProgram').controller('EducationProgramController', EducationProgramController);

    EducationProgramController.$inject = [
        '$rootScope',
        '$scope',
        'toastr',
        '$timeout',
    ...
    angular.module('Hrm.EducationProgram').directive('fileDownload',function(){
        return{
            restrict:'A',
            scope:{
                fileDownload:'=',
    ...
    angular.module('Hrm.EducationProgram').filter('removeHTMLTags', function() {
    ...
    function EducationProgramController($rootScope, $scope, toastr, $timeout, settings, utils, modal, service, Upload) {
        ...
        vm.getPage = function () {
            service.getPage(vm.searchDto,vm.pageIndex, vm.pageSize).then(function (data) {
                ...
                vm.bsTableControl.options.data = vm.educationPrograms;
            ...
            options: {
                ...
                onCheck: function (row, $element) {
                    $scope.$apply(function () {
                        vm.selectedEducationPrograms.push(row);
                    });
                },
                onCheckAll: function (rows) {
                    $scope.$apply(function () {
                        vm.selectedEducationPrograms = rows;
                    });
                },
                onUncheck: function (row, $element) {
                    ...
                    if (index >= 0) {
                        $scope.$apply(function () {
                            vm.selectedEducationPrograms.splice(index, 1);
                        });
                    }
                },
        ...
        vm.newObject = function () {
            ...
            vm.educationProgram.isNew = true;
            ...
            modalInstance.result.then(function (confirm) {
                if (confirm == 'yes') {
                    service.saveObject(vm.educationProgram, function success() {
                        vm.getPage();
                        toastr.info('Bạn đã tạo mới thành công một tài khoản.', 'Thông báo');
                        vm.educationProgram = {};
                    }, function failure() {
        ...
        $scope.editObject = function (id) {
            service.getOne(id).then(function (data) {
                vm.educationProgram = data;
                vm.educationProgram.isNew = false;
                var modalInstance = modal.open({
                    animation: true,
                    templateUrl: 'edit_object_modal.html',
                    scope: $scope,
        ...
        $scope.deleteObject = function (id) {
            ...
            modalInstance.result.then(function (confirm) {
                if (confirm == 'yes') {
                	console.log(vm.selectedEducationPrograms);
