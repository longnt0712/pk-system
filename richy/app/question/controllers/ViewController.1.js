    ...
    function ViewController($rootScope, $scope, toastr, $timeout, settings, utils, modal, service, $location,$stateParams,$window,blockUI,$sce,$cookies,topicService) {
        ...
        vm.getPageFlashCard = function () {
            ...
            service.getPageForGames(vm.searchDto, vm.searchDto.pageIndex, vm.searchDto.pageSize).then(function (data) {
                ...
                if(vm.searchDto.questionTopics !== null && angular.isDefined(vm.searchDto.questionTopics) && vm.searchDto.questionTopics.length > 0 ){
                    vm.searchDto.pageSize = data.totalElements;
                    vm.getQuestions();
                } else {
                    vm.searchDto.pageSize = 1;
                    vm.getQuestions();
        ...
        vm.clickNext = function () {
            ...
            // if(vm.currentPosition1 <= vm.questions.length){
            //     vm.currentPosition1 = vm.currentPosition1 + 1;
            //     vm.currentCard1 = vm.questions[vm.currentPosition1];
            //     vm.isShowDetail = false;
            //     vm.createQuiz(vm.currentCard1);
            // }
        };
                ...
                if((vm.currentPosition + 1) >= vm.searchDto.pageSize){
                    ...
                    if(vm.endGame == false) {
                        ...
                        if(player == 1){
                            ...
                            // window.speechSynthesis.speak(new SpeechSynthesisUtterance("1 wrong"));
                        }
                        if(player == 2){
                            vm.score2 = vm.score2 - 0.5;
                            // window.speechSynthesis.speak(new SpeechSynthesisUtterance("2 wrong"));
                        }
        ...
        vm.answerQuizBattle3 = function (correct,item,questions,player) { //player1 = 1 player 2 = 2
            ...
            if(correct == true){
                if(player == 1){
                    ...
                    if(vm.endGame == true){
                        window.speechSynthesis.speak(new SpeechSynthesisUtterance("TIME'S UP"));
                    } else if((vm.currentPosition1 + 1) >= vm.searchDto.pageSize && vm.endGame == false){
...
//         const synth = window.speechSynthesis;
        ...
        $scope.sayIt = function (text) {
            if (!vm.isMuted) {
                ...
                // // synth.getVoices();
                // // utterThis.voice = synth.getVoices()[6]; // 6 vietnamese
                // angular.forEach(synth.getVoices(), function (value, key) {
                //     if (value.lang === 'vi-VN') {
                //         utterThis.voice = value;
                //         vm.test = value;
                //         vm.test.length = synth.getVoices().length;
                //         // console.log(value);
                //     }
                // });
        ...
        vm.saveTestResult = function () {
            ...
            if(vm.mode.id == 8){
                ...
                vm.testResult.testTime = vm.totalCard.toString() +  ' WORD(s) | SCORE: '+ vm.score1.toString() + '| INCORRECT: ' + vm.wrongPlayer1.toString();
            }
            ...
            service.saveTestResult(vm.testResult).then(function (data1) {
                blockUI.stop();
                blockUI.start();
                service.getOneTestResult(data1.id).then(function (data) {
                    blockUI.stop();
