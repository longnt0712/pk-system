...
(function () {
    ...
    angular.module('Hrm.Answer').controller('AnswerController', AnswerController);
    ...
    angular.module('Hrm.Answer').directive('fileDownload',function(){
        return{
            restrict:'A',
            scope:{
                fileDownload:'=',
    ...
    angular.module('Hrm.Answer').filter('removeHTMLTags', function() {
    ...
    function AnswerController($rootScope, $scope, toastr, $timeout, settings, utils, modal, service, Upload) {
        ...
        vm.getPage = function () {
            service.getPage(vm.searchDto,vm.pageIndex, vm.pageSize).then(function (data) {
                ...
                vm.bsTableControl.options.data = vm.answers;
            ...
            options: {
                ...
                onCheck: function (row, $element) {
                    $scope.$apply(function () {
                        vm.selectedAnswers.push(row);
                    });
                },
                onCheckAll: function (rows) {
                    $scope.$apply(function () {
                        vm.selectedAnswers = rows;
                    });
                },
                onUncheck: function (row, $element) {
                    ...
                    if (index >= 0) {
                        $scope.$apply(function () {
                            vm.selectedAnswers.splice(index, 1);
                        });
                    }
                },
        ...
        vm.newObject = function () {
            ...
            modalInstance.result.then(function (confirm) {
                if (confirm == 'yes') {
                    service.saveObject(vm.answer, function success() {
                        vm.getPage();
                        toastr.info('Bạn đã tạo mới thành công một tài khoản.', 'Thông báo');
                        vm.answer = {};
                    }, function failure() {
