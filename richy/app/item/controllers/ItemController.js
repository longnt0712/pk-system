...
(function () {
    ...
    angular.module('Hrm.Item').controller('ItemController', ItemController);
    ...
    angular.module('Hrm.Item').directive('numberInput', function ($filter) {
        return {
            require: 'ngModel',
    ...
    angular.module('Hrm.Item').directive('myDatePicker', function () {
        return {
            restrict: 'A',
            require: '?ngModel',
    ...
    angular.module('Hrm.Item').directive('fileDownload',function(){
        return{
            restrict:'A',
            scope:{
                fileDownload:'=',
    ...
    angular.module('Hrm.Item').filter('removeHTMLTags', function() {
    ...
    function ItemController($rootScope, $scope, toastr, $timeout, settings, utils, modal, service, Upload) {
        ...
        vm.getPage = function () {
            service.getPage(vm.searchDto,vm.pageIndex, vm.pageSize).then(function (data) {
                ...
                vm.bsTableControl.options.data = vm.items;
        ...
        vm.newObject = function () {
            ...
            modalInstance.result.then(function (confirm) {
                if (confirm == 'yes') {
                    service.saveObject(vm.item, function success() {
                        vm.getPage();
                        toastr.info('Bạn đã tạo mới thành công một tài khoản.', 'Thông báo');
                        vm.item = {};
                    }, function failure() {
        ...
        vm.newListObject = function () {

            vm.item.isNew = true;

            var modalInstance = modal.open({
                animation: true,
                templateUrl: 'edit_list_object_modal.html',
                scope: $scope,
                size: 'md'
            });
            ...
            modalInstance.result.then(function (confirm) {
                if (confirm == 'yes') {
                    service.saveList(vm.item, function success() {
                        vm.getPage();
                        toastr.info('Bạn đã tạo mới thành công một tài khoản.', 'Thông báo');
                        vm.item = {};
                    }, function failure() {
        ...
        vm.newListObjectSticker = function () {
            ...
            var preCode = String(day)+String(month+1)+String(year);

            vm.item.dateReceipt = date;
            vm.item.code = preCode;

            var modalInstance = modal.open({
                animation: true,
                templateUrl: 'edit_list_object_sticker_modal.html',
                scope: $scope,
                size: 'md'
            });
            ...
            modalInstance.result.then(function (confirm) {
                if (confirm == 'yes') {
                    service.saveListSticker(vm.item, function success() {
                        vm.getPage();
                        toastr.info('Bạn đã tạo mới thành công một tài khoản.', 'Thông báo');
                        vm.item = {};
                    }, function failure() {
